    ...
    <a-menu style="width: 100%; border-right: 0;" mode="vertical" :selectedKeys="selectedKeys">
      ...
      >
        <div style="float:left; display:inline" >
            <img style="padding-top:10px;width: 60px;height: 60px" :src="item.imgUrl">
        </div>
...
export default {
      ...
      return {
        contact: this.contact.filter(item => {
          if (item && !this.blackList.includes(item.name)) {
            ...
            let p1 = this.zz(item.name)
            return Promise.all([p1]).then((result) => {
  ...
  methods: {
    ...
    select2(key, index) {
      ...
      console.log('this.$data.selectedKeys' ,this.$data.selectedKeys )
    ...
    zz(name){
      return new Promise((resolve, reject) => {
        this.$axios.get("/user/userImag/"+name).then(res=>{
