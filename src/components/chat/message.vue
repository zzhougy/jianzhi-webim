  ...
  <div class="messagebox" v-show="activedKey[type]!= ''">
    <div class="messagebox-header">
      ...
      <div>
        ...
        <span style="float: left;font-size: 20px;color: #333333;">{{`${activedKey[type].name } &nbsp;&nbsp; ${activedKey[type].groupid || ''}`}}</span>
...
<!--        >{{renderTime(item.time)}} {{item.bySelf?status[item.status]:''}}</div>-->
    ...
    <div style="border-top: 1px solid #ffffff;position: absolute;bottom: 2px;width: 100%;color        : rgba(0, 0, 0, 0.65);background   : #fff;">
      <el-button style="float: right;margin-right: 25px;margin-bottom: 10px" @click="onSendTextMsg">发送</el-button>
    </div>
